...
fn part1(grid: &[u8], width: usize) -> usize {
    ...
    let positions = (start..end).filter(|n| grid[*n] == SEAT_VACANT).collect::<Vec<usize>>();
    let tl_offset = usize::max_value()-width;
    let offsets = [tl_offset, tl_offset+1, tl_offset+2, usize::max_value(), 1, width - 1, width, width + 1];
